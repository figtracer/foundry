...
mod tests {
    ...
    fn build_eth_evm_with_extra_precompiles_osaka_spec() {
        ...
        assert!(evm.precompiles().addresses().any(|a| a == &ETH_OSAKA_PRECOMPILE));
        assert!(evm.precompiles().addresses().any(|a| a == &ETH_PRAGUE_PRECOMPILE));
        assert!(!evm.precompiles().addresses().any(|a| a == &PRECOMPILE_ADDR));
